...
### 1. Set 1: Direct Features (2 features) - Delicia
...
### 2. Set 2: Link-based Features (138 features) - Bernice
...
### 3. Set 3: Content-based Features (96 features) - Moto

你的这个要看一下参数，现在是cost = 1, gamma = 0.1；你看要不要在cha一下，找一下更好的参数
你的1+3的组合也是用的SVM。
...
# 4. Model Performance Comparison - Ren
...
cm_comparison %>%
  kable(caption = "Precision, Recall, F1-score Comparison (Single Split)") %>%
  kable_styling(bootstrap_options = c("striped", "hover"))
...
-   **Direct Features (Logistic Regression):**
    -   *Strengths:* Simple, interpretable, fast to train, and robust to overfitting with only two features. Provides clear probability outputs for ranking hosts by spamicity.
    -   *Weaknesses:* Limited by the small number of features; cannot capture complex or nonlinear relationships. Predictive power is constrained by the information content of the direct features.
    -   *Contextual Note:* In this dataset, direct features alone are insufficient for high-accuracy spam detection, as indicated by the lower AUC.
-   **Link-based Features (Random Forest):**
    -   *Strengths:* Handles high-dimensional data and complex, nonlinear relationships. Robust to outliers and can model feature interactions. Built-in feature importance helps interpretability. Random Forests are less sensitive to class imbalance with proper weighting.
    -   *Weaknesses:* Less interpretable than logistic regression. May require more computational resources. Performance can degrade if many features are irrelevant or highly correlated.
    -   *Contextual Note:* Link-based features provide richer information, and Random Forests leverage this to achieve higher AUC, making this approach effective for web spam detection.
-   **Content-based Features (SVM with RBF):**
    -   *Strengths:* SVMs are powerful for medium-dimensional, continuous data and can model complex, nonlinear boundaries. With class weighting, SVMs can handle imbalanced data. Good at capturing subtle content-based patterns.
    -   *Weaknesses:* Less interpretable, sensitive to parameter tuning (cost, gamma), and computationally intensive for large datasets. SVMs may struggle if features are not well-scaled or if there is noise.
    -   *Contextual Note:* Content-based features, when combined with SVM, can provide strong predictive power, but performance depends on the quality and relevance of the content features.

#### Overall Findings

-   **AUC as a Metric:** AUC is a robust measure for imbalanced classification problems, as it evaluates the model's ability to rank positive (spam) and negative (non-spam) cases across all thresholds.
-   **Best Feature Set:** The feature set with the highest AUC is the most effective for spam detection in this dataset. The results show that `r model_comparison$Feature_Set[which.max(model_comparison$CV_AUC)]` outperforms the others, likely due to the richness and relevance of its features.
-   **Visual ROC Comparison:** The ROC curves confirm the ranking, with the best model showing the greatest separation from the diagonal (random guess) line.

### Recommendations

-   For practical deployment, use the feature set and model with the highest AUC.
-   Consider combining feature sets or using ensemble methods for further improvement.
-   Address class imbalance with appropriate weighting or sampling if deploying on new data.

# 5. Feature Set Combinations: Model Deployment and Comparison

### 5.1 Combination 1: Obvious Features (Set 1) + Link-based Features (Set 2) ——Delicia
...
### 5.2 Combination 2: Obvious Features (Set 1) + Content-based Features (Set 3) --Moto
...
### 5.3 Combination 3: Link-based Features (Set 2) + Content-based Features (Set 3) -- Bernice
...
### 5.4 Combination 4: All Features (Set 1 + Set 2 + Set 3) --Ren
...
- **Obvious + Link**
  This combination utilizes both simple site-level metrics and structural link data. The AUC of 0.7662 is modest, but the recall (0.1402) is very low, meaning the model misses many spam cases. While Random Forest handles link features well, the obvious features contribute little, and the overall spam detection is weak.

- **Obvious + Content**
  The SVM (RBF) model captures subtle, non-linear textual patterns in content. Although the precision (0.3000) is slightly lower, the recall improves significantly to 0.3871, resulting in the highest F1-score (0.3380) among all combinations. This suggests it's relatively better at identifying spam correctly, even with fewer features.

- **Link + Content**
  This combination balances structural and content signals. Random Forest delivers the second-highest AUC (0.8240) and a better balance of precision (0.5152) and recall (0.1828) compared to previous sets. It shows robustness across spam strategies, although F1-score (0.2698) remains slightly behind SVM.

- **All Features**
  Using the full feature set yields the best overall AUC (0.8249) and the highest precision (0.5714). Recall (0.2151) improves over other Random Forest models, giving a strong F1-score of 0.3125. Although not the top in F1, this approach ensures stable, high-confidence predictions and is suitable when computational cost is acceptable.

⸻

🔍 **General Insights:**
- Combining Link + Content is crucial — this pairing consistently outperforms combinations that include only one of the two.
- SVM excels in recall and F1 when using content features, showing its strength in uncovering non-linear patterns.
- Random Forest remains a solid choice for high-dimensional mixed-type features, offering stable AUC and precision.
- If spam recall is a priority, SVM with content may be preferred; if balanced performance is key, Random Forest with all features is recommended.
